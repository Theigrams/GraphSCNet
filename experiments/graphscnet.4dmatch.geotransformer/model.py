...
from vision3d.ops import apply_deformation, build_euclidean_deformation_graph, index_select, pairwise_distance
...
class GraphSCNet(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        anchor_corr_indices, anchor_col_indices = torch.nonzero(anchor_masks, as_tuple=True)  # (S,), (S,)
        anchor_node_indices = corr_anchor_indices[anchor_corr_indices, anchor_col_indices]  # (S,)
        anchor_weights = corr_anchor_weights[anchor_corr_indices, anchor_col_indices]  # (S,)
        node_to_corr_weights = torch.zeros(size=(num_nodes, num_correspondences)).cuda()  # (M, C)
        node_to_corr_weights[anchor_node_indices, anchor_corr_indices] = anchor_weights  # (M, C)

        # 3. assign correspondences to nodes
        max_local_correspondences = torch.gt(node_to_corr_weights, 0.0).sum(dim=1).max().item()
        max_local_correspondences = min(max_local_correspondences, self.max_local_correspondences)
        ...
        local_corr_feats_norm = index_select(corr_feats_norm, local_corr_indices, dim=0)  # (M', k, d)
        ...
        if data_dict.get("registration", False):
            (
                ...
                edges_indices,
                edge_weights,
            ) = build_euclidean_deformation_graph(src_points, src_nodes, self.num_anchors, self.node_coverage)
            edge_weights = torch.ones_like(edge_weights)  # use the same weights for all edges
            ...
            warped_src_points = apply_deformation(src_points, src_nodes, transforms, anchor_indices, anchor_weights)
